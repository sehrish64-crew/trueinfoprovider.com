      ...
      <div className="flex items-end gap-2 h-48">
        {REVENUE_DATA.map((d, i) => (
          <motion.div
            ...
            className="flex-1 rounded-t-md bg-gradient-to-t from-emerald-600 to-emerald-400 relative group cursor-pointer min-w-0"
          >
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-white whitespace-nowrap bg-emerald-600 px-2 py-1 rounded shadow-sm">
        ...
        {activities.map((a, i) => (
          ...
          >
            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
              a.status === 'completed' ? 'bg-emerald-50' : a.status === 'processing' ? 'bg-blue-50' : 'bg-yellow-50'
            }`}>
              {a.status === 'completed' ? (
...
function ReportsTable() {
  const statusConfig = {
    delivered: { color: 'text-emerald-700', bg: 'bg-emerald-50', label: 'Delivered' },
    processing: { color: 'text-blue-700', bg: 'bg-blue-50', label: 'Processing' },
    refunded: { color: 'text-red-700', bg: 'bg-red-50', label: 'Refunded' },
  };
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-emerald-300 focus:ring-2 focus:ring-emerald-100 w-48"
          />
        </div>
      </div>
          ...
          <tbody>
            {REPORTS_TABLE.map((r, i) => {
              ...
              return (
                ...
                >
                  <td className="py-3 px-2 text-emerald-600 font-mono text-xs">{r.id}</td>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Monitor your AI Health Analysis platform</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-sm">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-emerald-700">{liveCount} scans today</span>
            </div>
